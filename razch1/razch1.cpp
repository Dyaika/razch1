#include <iostream>
#include <bitset>
using std::cin; using std::cout; using std::dec;
...
int main() {
    ...
    if (task > 0 and task < 6){
        ...
        cin >> num;
        cout << "Binary number is " << std::bitset<32>{num} << "\n";
    }
    switch (task) {
    case 1:
        mask = 0b101000000000000010;
        result = z1(num, mask);
        cout << "\nDecimal mask is " << mask << "\n";
        cout << "Binary mask is " << std::bitset<32>{mask} << "\n";
        break;
    case 2:
        mask = 0b111000;
        result = z2(num, mask);
    ...
    cout << "Binary result is " << std::bitset<32>{result} << "\n\n";
    return main();
}
